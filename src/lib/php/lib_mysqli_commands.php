 ...
 * in general:
		...
		lib_mysqli_interface::set('result',null);		// -> mysql-result-pointer, pointing to RAW mysql result of last query, no post-processing (sometimes you can not work directly with that), can be any type
		lib_mysqli_interface::set('output',null);		// -> data extracted from RAW mysql result, "the result" ready for further processing, can be any type
		lib_mysqli_interface::set('feedback','');		// -> contains message to client e.g. the last detailed success/error message, it is structured like this: "type:error,id:unique_id_of_feedback_message,details:"." Selecting database failed: ".mysqli_connect_error() so the JavaScript-client can display it
		...
		lib_mysqli_interface::set('worked',false);		// -> this is the status of the last query possible values are true (worked) false (failed, mysql error will be given in 'feedback' and thrown at JavaScript (or any other) client)
		lib_mysqli_interface::set('last_id','');		// -> if there was an insert, return the auto-generated id of the record inserted.
...
class lib_mysqli_commands extends lib_mysqli_interface {
	...
	* */
	public function describe($table,$mode = "object")
	{
		...
		$tableDefinition = config::get('lib_mysqli_interface_instance')->query("DESCRIBE ".$table);
		...
		if($target == 0)
		{
			...
			lib_mysqli_interface::set('worked',true);
		}
		else
		{
			for($i=0;$i<$target;$i++)
	...
	* effectively linking your group-Objects-layout to the database */
	...
	public function UserExist($user,$uniqueKey = "id")
	{
		...
		if($user->$uniqueKey)
		{
			...
			$user_array = config::get('lib_mysqli_interface_instance')->query($query);
		
			if(empty($user_array))
			{
				lib_mysqli_interface::set('output',false);
			}
			else
			{
				lib_mysqli_interface::set('output',true);
			}
		}
		else
		{
		...
		return lib_mysqli_interface::get('output');
	}
	...
	*/
	public function users($user = null,$uniqueKey = "id",$uniqueValue = null,$where = "")
	{
		...
		// execute sql query
		$temp = config::get('lib_mysqli_interface_instance')->query($query);
	
		if(!empty($temp)) lib_mysqli_interface::set('worked',true);
	...
	*/
	public function groups($group = null,$uniqueKey = "id",$where = "")
	{
		...
		$group_array = config::get('lib_mysqli_interface_instance')->query($query);
	...
	/* 
	* save an sessionID to browser as cookie and also to mysql database
	*
	* lifetime:
	* SESSIONS ENDS WHEN USER CLOSE HIS BROWSER, you’ve configured php.ini to extend sessions’ cookie lifetime. Cannot last forever.
	* 
	* COOKIES ENDS DEPENDING ON THE LIFE TIME YOU SET FOR IT. SO IT CAN LAST FOR YEARS (conveniant, but then you will forget your password)
	*
	* Cookie or no Cookie?
	* session is actually setting a COOKIE? -> Session is dependent on COOKIE.
	* 
	...
	*/
	public function SetCookie($username,$cookie = false)
	{
		// when the user logged in (ms since 1.1.1970
		$logintime = time();
	
		// store to database how long the session should last
		$valid_until = time()+(config::get('login_session_timeout')*1000); // convert from seconds to milliseconds

		$salt = salt();
		setcookie("hyperhelp", $salt, $valid_until);  /* expire in 1 hour */

		// save sessionID in database 
		config::get('lib_mysqli_interface_instance') -> query("UPDATE `".config::get("db_name")."`.`".config::get("db_auth_table")."` SET `logintime` = '".$logintime."', `loginexpires` = '".$valid_until."', `session` = '".$salt."' WHERE `".config::get("db_auth_table")."`.`username` = '".$username."';");
		
		// increment LoginCount
		config::get('lib_mysqli_interface_instance') -> query("UPDATE `".config::get("db_name")."`.`".config::get("db_auth_table")."` SET LoginCount = LoginCount + 1 WHERE `".config::get("db_auth_table")."`.`username` = '".$username."';");
	...
	/*
	 * get $session
	* $session hash is sha512($username.$password.$salt)
	* returns timestamp until when the session is valid
	*/
	public function GetCookieExpiration($session, $user)
	{
		$valid_until = null;
		...
		if ($user == null) {
			...
			$user = GetFirstElementOfArray($user);
		}
	...
	public function GetUserBySession($session)
	{
		...
		{
			$valid_until = null;
			$user_array = config::get('lib_mysqli_interface_instance')->query("SELECT * FROM `".config::get("db_auth_table")."` WHERE `session` = '".$session."'");
	...
	* */
	public function UserDel($user,$IdentifyBy = "id")
	{
		...
		{
			$temp = config::get('lib_mysqli_interface_instance')->query("DELETE FROM  `".config::get("db_name")."`.`".config::get("db_auth_table")."` WHERE `".config::get("db_auth_table")."`.`".$IdentifyBy."` = '".$user->$IdentifyBy."';");
			lib_mysqli_interface::set('worked',true);
		}
	...
	* */
	public function UserAdd($user) // $requested_username = "",$requested_password = "",$groups = "",$data = ""
	{
		...
		{
			return error("function UserAdd: can not continue, user ".$user->username." is taken.");
		}

		// Create a unique activation code:
		$user->activation = generate_activation_key();
		...
		$user->password = password_encrypt($user->password); // encrypt password, before storing it in database
		...
		$user->id = lib_mysqli_interface::get('last_id');
	...
	* data -> $data = "key:value,key:value," */
	public function UserEdit($UpdatedUser,$uniqueKey = "id") // $userID, $requested_username = "",$requested_password = "",$groups = "",$data = ""
	{
		...
		config::get('lib_mysqli_interface_instance')->query($query); // its an UPDATE sql command, so no result except "success" expected.
	
		return lib_mysqli_interface::set('worked',true);
	}
	...
	* */
	public function GroupAdd($group,$systemgroup = 0)
	{
		...
		{
			lib_mysqli_interface::set('worked',false);
		...
		if(!$this->GroupExist($group))
		{
			...
			$group->id = lib_mysqli_interface::get('last_id');
	...
	public function GetGroupByName($groupname)
	{
		...
		if($temp_group)
		{
			// lib_mysqli_interface::set('worked',true); // is already set to true
			lib_mysqli_interface::set('output',$temp_group);
			return $temp_group;
		}
		else
		{
			lib_mysqli_interface::set('worked',false);	// is already set?
			lib_mysqli_interface::set('output',null);	// is already set?
	 ...
	 * 1. need to $group = config::get('lib_mysqli_interface_instance')->GetGroupByName("groupname");
	...
	*/
	public function GroupUpdate($group)
	{
		...
		if($group2change)
		{
			...
			config::get('lib_mysqli_interface_instance')->query($query); // run the query
	...
	public function GroupDel($group,$IdentifyBy = "id")
	{
		...
		if(haspropertyandvalue($group,$IdentifyBy,"GroupDel"))
		{
			...
			else
			{
				...
				$result = config::get('lib_mysqli_interface_instance') -> query($query);
			}
		}
	...
	*/
	...
	{
		$result = config::get('lib_mysqli_interface_instance')->query("SELECT * FROM `".config::get("db_groups_table")."`");
	...
	* */
	public function GroupExist($group,$uniqueKey = "groupname")
	{
		...
		lib_mysqli_interface::set('output',$temp);
		return $temp;
	}
	...
	/* add/register a new record, then return it including it's newly created database-autoincremented-id
	 *
	...
	* */
	public function RecordAdd($table = null,$record) // $requested_recordname = "",$requested_password = "",$groups = "",$data = ""
	{
		...
		$record->id = lib_mysqli_interface::get('last_id');
	...
	public function RecordEdit($table = null,$Record2Update,$uniqueKey = "id")
	{
		...
		config::get('lib_mysqli_interface_instance')->query($query);

		return $Record2Update->id; // lib_mysqli_interface::get('worked');
	}
	...
	*/
	...
	public function ImportSQLFromFile($url)
	{
		...
		else
		{
			...
			for($i = 0;$i<$count;$i++)
			{
				...
				{
					config::get('lib_mysqli_interface_instance')->query($line); // execute this line
				}
				else if(($cmd6 == "INSERT") || ($cmd6 == "CREATE"))
				{
					...
					config::get('lib_mysqli_interface_instance')->query($multiline); // execute this line
				}
			}
		}
	...
	public function DatabaseExists($databaseName)
	{
		...
		$allDatabaseNames = lib_mysqli_interface::query($query);

		if(lib_mysqli_interface::get('worked')) // proceed if last query was successful and returning data
		{
	...
	public function TableExists($databasename,$tablename)
	{
		...
		$AllTablesOfDatabase = lib_mysqli_interface::query($query);
